...
pkg <- c("stringr","stringi","tidyverse","foreign","conflicted","data.table","raster","gtools",
         "ggplot2","ggpubr","rstudioapi","scales","splitstackshape","devtools",
         "tidyr","devtools", "sf", "geojsonio","lme4","arm","MASS","knitr","wru")
...
suffix_vec <- c(" IV", " III", " II", " Jr", " Sr")

## loop through to get rid of suffixes 
for (i in 1:length(suffix_vec)) {
  police$full_name <- 
    str_replace_all(police$full_name,suffix_vec[i], "" )
...
police$first_name <- sapply(strsplit(police$full_name, " "), `[`, 1) 
# for first name, we are grabbing the portion that is first before the space split. 
...
print(summary(police$name_length))  
...
police_df_miss_surname <- predict_race(police_df_miss_surname,
                                       surname.only = T)
